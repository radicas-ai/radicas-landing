// Proposal 2 — Trust & compliance band. Honest about shipped vs. roadmap.
// Monoline icons in periwinkle; restrained Carbon cards; no accent confetti.

type Status = "shipped" | "roadmap";
type Trust = { title: string; body: string; status?: Status; icon: string };
...
function StatusTag({ status }: { status: Status }) {
  if (status === "shipped") {
    return (
      <span className="rounded px-1.5 py-0.5 font-mono text-2xs font-semibold uppercase tracking-caps text-semantic-allow bg-semantic-allow/12">
        Shipped
      </span>
    );
  }
  return (
    <span className="rounded px-1.5 py-0.5 font-mono text-2xs font-semibold uppercase tracking-caps text-semantic-review bg-semantic-review/12">
      In progress
    </span>
  );
}
